...
|---|---|---|
| [DRAM Capacitor DOE, SPC & AI-Assisted Root-Cause Analysis](https://github.com/Joshi-Shreyas/dram-capacitor-doe-spc) | Simulated a DRAM MIM capacitor process end-to-end: built a physics-grounded model of capacitance/leakage/reliability tradeoffs, ran a Design of Experiments to find the optimal recipe, monitored simulated production with statistical process control, built a multivariate anomaly detector that catches excursions univariate charts miss, and used an LLM to auto-generate root-cause investigation reports.| Python · pyDOE3 · statsmodels · SciPy · pandas · Matplotlib · Google Gemini API |
