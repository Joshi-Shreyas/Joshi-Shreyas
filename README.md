...
|---|---|---|
| [Plasma Etch Bayesian Optimization](https://github.com/Joshi-Shreyas/plasma-etch-bayesian-optimization) | GP-based BO for semiconductor etch recipe targeting in 4D parameter space — 0.81 ± 0.55 Å/min mean deviation from target, outperforms 3-level factorial DOE and random search, multi-objective Pareto front via qLogNEHVI | BoTorch · GPyTorch · PyTorch · Python |
...
| LingBot-VA: Vision-Action Robot Control (progress) | INT8 quantization of a vision-action transformer for robot manipulation. 49% inference speedup, 17.2% VRAM reduction, 43.8% to 87.5% task success on open_microwave benchmark. Evaluated on A100 GPUs. | PyTorch · INT8 Quantization · A100 HPC |
